...
LPVOID recving_scheme ()
{
	...
	if (c_noise != 0.0)
	{
		...
		int c_threshold = (1.0 - pow (1.0 - c_noise, p_socket_recved_raw_new->size * f_fraction)) * 32768.0 + 0.5;
		if (rand () <= c_threshold)
		{
	...
	LPVOID result = p_socket_recved_raw;
...
int sending_scheme ()
{
	static int c_last_time_sending;

	int result = c_current_time;

	int c_socket_buf_up_tmp;

	int c_actual_to_send;
	int c_actually_sent;
	...
	if (c_last_time_sending)
	{
		if (c_current_time > c_last_time_sending)
		{
			...
			c_tmp =
				((UINT64) (274877907i64 * (8000 * (c_current_time - c_last_time_sending) / 8)) >> 32) >> 6;
...
int is_packet_ready ()
{
	static int c_last_time_checking;
