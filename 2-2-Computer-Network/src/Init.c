...
inline void memset32 (void *dest, UINT32 value, UINT32 size)
{
	UINT i;
...
tag_Read_FirstPart_Done:
	...
	if (!some_struct.str[0])
	{
		...
		if (!_strcmpi (&some_struct.str[strlen (some_struct.str) - 4], ".eXe"))
	...
	if (argc > 3)
	{
		...
		while (1)
		{
			if (_strcmpi (*arg_i, "-ber"))
			{
				if (_strcmpi (*arg_i, "-life"))
				{
					if (_strcmpi (*arg_i, "-idle"))
					{
						if (_strcmpi (*arg_i, "-tick"))
						{
							if (_strcmpi (*arg_i, "-port"))
							{
								if (_strcmpi (*arg_i, "-debug"))
								{
									if (_strcmpi (*arg_i, "-log"))
									{
										argv_copy = argv;
										goto tag_Err_Argument;
									}
	...
	if (!_strcmpi (some_struct.str, "nul") || !_strcmpi (some_struct.str, "null"))
		sprintf (some_struct.str, "NUL");
	p_Log_File = fopen (some_struct.str, "w");

	if (!p_Log_File)
	{
		int *c_err_code = _errno ();
