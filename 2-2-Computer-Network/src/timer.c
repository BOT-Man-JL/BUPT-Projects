...
{
	int c_timer_id = 0;
	int *p_timers = array_timer;

	while (!*p_timers || *p_timers > c_current_time)
	{
		++p_timers;
		++c_timer_id;
		...
		//         Seperating the vars
		//if (p_timers >= &c_recv_total_bit)
		//	return 0;

		// Added by BOT Man JL :)
		if (c_timer_id >= MAX_TIMER_COUNT)
			if (c_ack_timeout > c_current_time)
			{
				*arg_ret = c_timer_id;
				c_ack_timeout = 0;
				return ACK_TIMEOUT;
	...
	// Reset the timer
	*arg_ret = c_timer_id;
	array_timer[c_timer_id] = 0;

	// return timeout type
	return (c_timer_id == MAX_TIMER_COUNT) + 3;
}
