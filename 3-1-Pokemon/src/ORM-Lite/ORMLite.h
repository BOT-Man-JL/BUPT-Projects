...
namespace BOT_ORM_Impl
{
	...
	{ TupleVisitor_Impl (tuple, action, _SizeT<std::tuple_size<TupleType>::value> ()); }

	template < typename TupleType, typename ActionType >
	inline void TupleVisitor_Impl (TupleType &tuple, ActionType action, _SizeT<0>) {}

	template < typename TupleType, typename ActionType, size_t N >
	inline void TupleVisitor_Impl (TupleType &tuple, ActionType action, _SizeT<N>)
	{
...
namespace BOT_ORM
{
	...
	public:
		...
		typename std::enable_if<std::is_default_constructible<C>::value, bool>::type
			CreateTbl ()
		{
			C value {};
		...
		bool CreateTbl (const C &value)
		{
			...
			{
				size_t index = 0;
				std::vector<std::string> strTypes (ORMapper<C>::_FieldNames ().size ());
				auto tuple = value.__ValTuple ();
				BOT_ORM_Impl::TupleVisitor (tuple,
											[&strTypes, &index] (auto &val)
				{
					strTypes[index++] = BOT_ORM_Impl::TypeString (val);
				});

				auto strFmt = ORMapper<C>::_FieldNames ()[0] + " " + strTypes[0] +
					" primary key,";
				for (size_t i = 1; i < ORMapper<C>::_FieldNames ().size (); i++)
					strFmt += ORMapper<C>::_FieldNames ()[i] + " " + strTypes[i] + ",";
				strFmt.pop_back ();
		...
		bool Update (const C &value)
		{
				...
				BOT_ORM_Impl::SQLConnector &connector)
			{
				...
				auto tuple = value.__ValTuple ();
				BOT_ORM_Impl::TupleVisitor (tuple,
											[&os, &osKey, &index] (auto &val)
				{
					if (index == 0)
					{
						osKey << ORMapper<C>::_FieldNames ()[index++] << "=";
						BOT_ORM_Impl::SerializeValue (osKey, val);
					}
					else
					{
						os << ORMapper<C>::_FieldNames ()[index++] << "=";
						if (index != ORMapper<C>::_FieldNames ().size ())
							BOT_ORM_Impl::SerializeValue (os, val) << ',';
						else
							BOT_ORM_Impl::SerializeValue (os, val);
				...
				BOT_ORM_Impl::SQLConnector &connector)
			{
								   ...
								   [&] (int argc, char **argv, char **)
				{
					if (argc != ORMapper<C>::_FieldNames ().size ())
						throw std::runtime_error ("Inter ORM Error: Select argc != Field Count");
